...
const slides = [
  ...
  { key: "compositionExpand", chapter: "03 · 构图扩展", tab: "扩图构图", title: "主体不变，重新安排景别与留白", desc: "把近景扩展为完整竖版构图，补足环境和氛围。", status: "案例：解决尺寸不合适与构图太挤", ticker: "一张素材继续适配海报、封面和不同平台比例。", fallback: "很多图片内容不错，却因为比例不对、主体太满，无法直接用于海报或竖版封面。灵图AI可以保留主体和原有风格，向外扩展环境，重新安排人物位置、留白和视觉动线。这样一张素材就能适配小红书封面、短视频竖图和商业海报。二K一张只要两分钱，当前还可以免费试用，关注后即可领取。" },
  ...
  { key: "productDetail", chapter: "08 · 电商素材", tab: "商品详情", title: "一套商品图，延展主图、详情与营销素材", desc: "统一包装、卖点、背景和展示角度，减少外包与返工。", status: "解决问题：电商素材多、制作周期长", ticker: "适合抖店、淘宝、私域商品和新品测试。", fallback: "电商商家需要的不只是一张主图，还包括详情页、包装展示、卖点图和活动素材。灵图AI可以围绕同一商品连续生成和修改，让视觉风格保持统一，也减少不同设计师之间的反复沟通。二K只要两分钱，批量测试方案的成本非常低。当前免费试用，关注后可以直接体验完整流程。" },
