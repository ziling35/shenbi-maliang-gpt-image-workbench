...
export function saveObjectStorageSettings(input: Partial<ObjectStorageSettings>) {
  ...
  run(configDb, `insert into object_storage_settings(id,enabled,secret_id,secret_key,bucket,region,base_path,public_base_url,updated_at)
    values('default',?,?,?,?,?,?,?,?) on conflict(id) do update set enabled=excluded.enabled,secret_id=excluded.secret_id,secret_key=excluded.secret_key,bucket=excluded.bucket,region=excluded.region,base_path=excluded.base_path,public_base_url=excluded.public_base_url,updated_at=excluded.updated_at`,
  enabled ? 1 : 0,
...
function usesCos(relativePath: string, settings = objectStorageSettings(true)) {
  ...
  return settings.enabled && (path.startsWith("files/secure/images/") || path.startsWith("files/secure/derivatives/"));
}
...
export async function getStoredObjectUrl(relativePath: string, expires = 300) {
  ...
  try {
    await cosRequest((callback) => cosClient(settings).headObject({
